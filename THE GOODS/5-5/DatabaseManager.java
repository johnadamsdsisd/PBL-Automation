...
public class DatabaseManager {
   ...
   public ArrayList<Card> getAllCards() {
      try {
         ...
         while (rs.next()) {
            temp.add(new Card(rs.getInt(ID), rs.getString(TITLE), rs.getString(TYPE), rs.getString(DESC), getColor(rs.getString(TYPE)), rs.getInt(MIN), rs.getInt(MAX), rs.getString(ATTRS), 1));
         }
         return temp;
      } catch (SQLException sqe) {
         return null;
   ...
   private String getDataFromSelection(ResultSet rs) throws SQLException {
      ...
      while (rs.next())
         result += rs.getInt(ID) + "  % " + rs.getString(TITLE) + " % " + rs.getString(TYPE) + " % " + rs.getString(DESC) + " % " + getColor(rs.getString(TYPE)) + " % " + rs.getInt(MIN) + " - " + rs.getInt(MAX) + " % " + rs.getString(ATTRS) + "%\n";
      rs.close();
      return result;
   }
